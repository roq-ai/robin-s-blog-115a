...
import { getArchiveById } from 'apiSdk/archives';
import { ArchiveInterface } from 'interfaces/archive';

function ArchiveViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('archive', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/archives/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('picture', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Picture"
                    text={
                      <Link as={NextLink} href={`/pictures/view/${data?.picture?.id}`}>
                        {data?.picture?.title}
                      </Link>
                    }
                  />
