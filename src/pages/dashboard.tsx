...
import { getArchives } from 'apiSdk/archives';
import { ArchiveInterface } from 'interfaces/archive';
import { getPictures } from 'apiSdk/pictures';
...
function DashboardPage() {
  ...
  const archiveStat = useSWR<PaginatedInterface<ArchiveInterface>>(
    () => `/archives`,
    () => getArchives({ limit: 1, offset: 0 }),
  );

  const pictureStat = useSWR<PaginatedInterface<PictureInterface>>(
    () => `/pictures`,
    () => getPictures({ limit: 1, offset: 0 }),
  );

  const albumStat = useSWR<PaginatedInterface<AlbumInterface>>(
    () => `/albums`,
    () => getAlbums({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Archives',
      result: archiveStat?.data?.totalCount,
      isLoading: archiveStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Pictures',
      result: pictureStat?.data?.totalCount,
      isLoading: pictureStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Albums',
      result: albumStat?.data?.totalCount,
      isLoading: albumStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
